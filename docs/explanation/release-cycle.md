...
`self.version` is a Composer convention meaning "the same version as me". It works because the umbrella coordinates wave releases — `security@0.1.0-beta5` requires `contracts@0.1.0-beta5`, `routing@0.1.0-beta5`, etc. The whole wave is internally consistent.

The cost: you cannot, in practice, mix-and-match component versions across a wave. If you want `security@0.1.0-beta5` you implicitly want every other component at `0.1.0-beta5`. This is the explicit design — coordinated waves over fine-grained interop.

## The release wave, step by step

A coordinated wave (e.g. `0.1.0-beta4` → `0.1.0-beta5`):
